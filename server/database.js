...
const db = {
  ...
  reviews: [
    ...
    },
    {
      id: "REV-1003",
      targetType: "hotel",
      target: "Lotus Bay Goa",
      author: "Maya P",
      rating: 3,
      text: "Good pool and location, but check-in took longer than expected during peak weekend.",
      photos: [],
      helpful: 9,
      createdAt: "2026-04-23T18:40:00.000Z",
      flagged: false,
      replies: []
    }
  ],
...
function dynamicPricing() {
  ...
  const demandMultiplier = 1 + db.flights.filter((flight) => flight.status !== "On Time").length * 0.04;
  const seasonalMultiplier = isWeekend ? 1.2 : 1.08;
  const inventoryMultiplier = db.selections.seats.filter((seat) => seat.available).length < 50 ? 1.12 : 1;
  return {
    base,
...
function tickFlightStatuses() {
  ...
  const flight = db.flights[Math.floor(Math.random() * db.flights.length)];
